...
router.post(
  ...
  [
    ...
    body("dataInizioDisponibilita")
      ...
      .isAfter(validazione.dataLimite)
    ...
    body("oraInizioMattina")
      ...
      .custom(async (oraInizioMattina, { req }) => {
        ...
        let oraIMatt = new Date().setHours(oraInizioMattina.split(":")[0], oraInizioMattina.split(":")[1], 0 ,0)
        let oraFineMattina = new Date().setHours(req.body.oraFineMattina.split(":")[0], req.body.oraFineMattina.split(":")[1], 0, 0 );
       ...
       if(oraIMatt < validazione.orarioMattinaLimite)
       {
          ...
          throw new Error("'\Ora inizio mattina'\ non può essere più grande di '\Ora fine mattina'\!");
        }
      }),
    body("oraFineMattina")
      .matches(validazione.orario)
      ...
      .custom(async (oraFineMattina, { req }) => {
        ...
        let oraFMatt = new Date().setHours(oraFineMattina.split(":")[0], oraFineMattina.split(":")[1], 0, 0 );
        let oraInizioPomeriggio = new Date().setHours(req.body.oraInizioPomeriggio.split(":")[0], req.body.oraInizioPomeriggio.split(":")[1], 0, 0);

       if(oraFMatt >= oraInizioPomeriggio)
       {
          throw new Error("'\Ora Fine Mattina'\ non può essere >= di '\Ora Inizio Pomeriggio'\!");
       } 
    ...
    body("oraInizioPomeriggio")
      ...
      .custom(async (oraInizioPomeriggio, { req }) => {
        ...
        let oraIPom = new Date().setHours(oraInizioPomeriggio.split(":")[0], oraInizioPomeriggio.split(":")[1], 0, 0);
        let oraFineMattina = new Date().setHours(req.body.oraFineMattina.split(":")[0], req.body.oraFineMattina.split(":")[1], 0 ,0 );
        let oraFinePomeriggio = new Date().setHours(req.body.oraFinePomeriggio.split(":")[0], req.body.oraFinePomeriggio.split(":")[1], 0, 0 );

       if(oraIPom <= oraFineMattina)
       {
          throw new Error("'\Ora Inizio Pomeriggio'\ non può essere <= di '\Ora Fine Mattina'\!");
       }
       else if(oraIPom >= oraFinePomeriggio){
        throw new Error("'\Ora Inizio Pomeriggio'\ non può essere >= di '\Ora Fine Pomeriggio'\!");
       }
    ...
    body("oraFinePomeriggio")
      ...
      let oraFPom = new Date().setHours(oraFinePomeriggio.split(":")[0], oraFinePomeriggio.split(":")[1], 0, 0);
      ...
      body("dateChiusura")
      .custom(async (dateChiusura) => {
        let dateChiusuraArray = JSON.parse(dateChiusura).dateChiusura;

        dateChiusuraArray.forEach(dataChiusura => {
          if(!dataChiusura.match(validazione.data) || (isNaN( Date.parse(dataChiusura))) 
          || Date.parse(dataChiusura) < Date.parse(validazione.dataLimite) ){
